    ...
    <div>
      <Breadcrumb>
        ...
        <Link to="/account/overzicht">
          <Breadcrumb.Section link>Accountoverzicht</Breadcrumb.Section>
        </Link>
      </Breadcrumb>
      <Divider />
    </div>
  ...
  render() {
    ...
    const panes = [
      ...
      },
      {
        ...
        render: () => { if(!this.state.infoLoaded) {
            return <LoaderCenter />; }
            else return( <PersonalInfoOverview data={this.state.personalInfo} />);
...
const LoaderCenter = () => (
  <Loader active inline="centered" style={{ paddingTop: "2em" }} />
);
