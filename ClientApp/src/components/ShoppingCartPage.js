...
class ShoppingCart extends Component {
  ...
  handleCodeChange = (e, { value }) =>
    this.setState({ ...this.state, discount: { code: value }, display: false });

  handleDiscount = () => {
    fetch('/order/SearchDiscount?input=' + this.state.discount.code).then(
      results => {
        if (!results.ok) {
          this.setState(
            {
    ...
    if (this.state.display && this.state.error) {
      message = (
        <div>
          <Message negative>
            <Message.Header>
              De ingevulde kortingcode is niet geldig!
            </Message.Header>
                  ...
                  <Grid.Row key={product.id}>
                    <Grid.Column width={2}>
                      <Link to={'/product/' + product.id}>
                        <Image src={product.url} className="shoppingCartImage"/>
                      </Link>
                    </Grid.Column>
                    <Grid.Column width={4}>
                      <Link to={'/product/' + product.id}>
                    ...
                    <Grid.Column width={2}>
                      Prijs: €{String(product.price).replace('.', ',')}
                    </Grid.Column>
                ...
                {this.state.discount.validated ? (
                  <div>
                    <h4>
                      ...
                      {String(
                        parseFloat(
                          Math.round(this.props.shoppingcart.totalPrice * 100) /
                            100
                        ).toFixed(2)
                      ).replace('.', ',')}
                    </h4>
                    {this.state.discount.procent === true ? (
                      <div>
                        <h4>
                          Korting:{' '}
                          {String(this.state.discount.amount).replace('.', ',')}
                          %
                        </h4>
                        <h3>
                          Totaal: €{' '}
